...
public class Statuspage {
    ...
    public static void CheckMessage(Message message) {
        ...
        if (rawContent.contains("UP")) {
            message.getChannel()
                    ...
                    .setActionRow(
                            Button.success("operational-" + serviceName, "Operacional"))
                    .queue();
            return;
        }
        ...
        message.getChannel()
                ...
                .setActionRow(
                        Button.primary("under_maintenance-" + serviceName,
                                "Settar Manutenção"),
                        Button.danger("major_outage-" + serviceName,
                                "Settar Major Outage"),
                        Button.secondary("partial_outage-" + serviceName,
                                "Settar Partial Outage"),
                        Button.secondary("degraded_performance-" + serviceName,
                                "Settar Degraded Performance"),
                        Button.link("https://menherabot.xyz/status",
                                "Status Page"))
                .queue();
    ...
    private static void sendStatusPageRequest(String componentId, ComponentStates state) {
        ...
        try {
            ...
            logger.error("Error sending statuspage request", e);
        }
    }
}
