...
public class Statuspage {
  ...
  public static void CheckMessage(Message message)  {
    ...
    if(rawContent.contains("UP")) {
      message.getChannel().sendMessage("O serviço **" + serviceName + "** está pingando novamente, settar o status como operacional?")
      .setActionRow(
        net.dv8tion.jda.api.interactions.components.buttons.Button.success("operational-"+serviceName, "Operacional")
      ).queue();
      return;
    }

      message.getChannel().sendMessage("**SERVIÇO FORA DO AR**\n\n **" + serviceName + "** - Não está pingando. Estado settado como _Major Outage_\n\n<@"+Constants.OWNER_ID+">")
      .setActionRow(
        net.dv8tion.jda.api.interactions.components.buttons.Button.primary("under_maintenance-"+serviceName, "Settar Manutenção"),
        net.dv8tion.jda.api.interactions.components.buttons.Button.danger("major_outage-"+serviceName, "Settar Major Outage"),
        net.dv8tion.jda.api.interactions.components.buttons.Button.secondary("partial_outage-"+serviceName, "Settar Partial Outage"),
        net.dv8tion.jda.api.interactions.components.buttons.Button.secondary("degraded_performance-"+serviceName, "Settar Degraded Performance"),
        net.dv8tion.jda.api.interactions.components.buttons.Button.link("https://menherabot.xyz/status", "Status Page")
      ).queue();
